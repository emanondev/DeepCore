		...
		protocolManager.addPacketListener(
				new PacketAdapter(CoreMain.get(), ListenerPriority.NORMAL, PacketType.Play.Client.ITEM_NAME) {
					...
					public void onPacketReceiving(PacketEvent event) {
						if (!(event.getPlayer().getOpenInventory().getTopInventory().getHolder() instanceof TextGui))
							return;
						TextGui gui = (TextGui) event.getPlayer().getOpenInventory().getTopInventory().getHolder();
