...
public class UtilsString {
	 ...
	 * @param color   translate colors
	 * @param holders additional place Holders, must be even number with the format
	 ...
	 * @param player      player or null for placeHolderApi use
	 * @param color       translate colors
	 * @param holders     additional place Holders, must be even number with the
	...
	public static @NotNull List<String> textLineSplitter(@Nullable String text, int maxLenght) {
		return textLineSplitter(text,maxLenght,ChatColor.WHITE);
	}

	public static @NotNull List<String> textLineSplitter(@Nullable String text, int maxLenght,@Nullable ChatColor base) {
		...
		for (int index = 0; index < words.length; index++) {
			if (wordCounter > 0 && ChatColor.stripColor(line.toString()).length() + 1
					+ ChatColor.stripColor(words[index]).length() > maxLenght) {
				val.add(format.toString()+line.toString());
				int start = 0;
				while (start < line.toString().length()) {
					...
					if (j == -1)
						break;
					try {
						format.append(line.toString().substring(j, j + 2));
					} catch (Exception e) {
						e.printStackTrace();
					}
					start = j + 2;
