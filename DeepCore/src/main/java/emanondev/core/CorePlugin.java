...
public abstract class CorePlugin extends JavaPlugin implements ConsoleLogger{
	...
	private final EnumMap<CounterAPI.ResetTime, CounterAPI> counterApiMap = new EnumMap<CounterAPI.ResetTime, CounterAPI>(
	...
	public final void onLoad() {
		...
		try {
			// TODO bad
			new File(this.getDataFolder(), "permissions.yml").delete();
		} catch (Exception e) {
			e.printStackTrace();
		}
		load();
	...
	public final void onDisable() {
		...
		for (Player p : Bukkit.getOnlinePlayers())
			if (p.getOpenInventory().getTopInventory().getHolder() instanceof Gui
				...
				p.closeInventory();
				this.logDone("Safetly closing gui for &e" + p.getName() + " &fto prevent dupe glitches");
			}
		try {
			disable();
		} catch (Exception e) {
		...
		for (SQLDatabase conn : new HashSet<>(connections))
			try {
				...
				logDone(conn.getType().name() + " succesfully dicconnected from &e" + conn.getUrl());
	 ...
	 * Should inizialize static instance of the plugin.<br>
	 ...
	 * @param command
	 *            Command to register
	 * @return true if command is succefully registered
	 * @see CoreCommand
	...
	public boolean registerCommand(@NotNull Command command) {
		try {
			...
			if (!commandMap.register(this.getName().toLowerCase(), (Command) command))
	...
	public SQLDatabase createConnection(SQLType type, String databaseName, String host, int port, String username,
		...
		try {
			...
			logDone(type.name() + " succesfully connected to &e" + con.getUrl());
			return con;
		} catch (Exception e) {
